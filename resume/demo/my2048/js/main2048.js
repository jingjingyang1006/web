...
function init(){
    ...
    for( var i = 0 ; i < 4 ; i ++ ){
        ...
        for( var j = 0 ; j < 4 ; j ++ ){
            board[i][j] = 0;
            hasConflicated[i][j]=false;//初始化成false，默认每个位置没有发生碰撞；
        }
    }
...
function updateBoardView(){
    ...
    for( var i = 0 ; i < 4 ; i ++ )
        for( var j = 0 ; j < 4 ; j ++ ){
            ...
            hasConflicated[i][j]=false;//新的一轮开始，碰撞的值进行一次归位；
        }
...
document.addEventListener("touchend",function(event){
    ...
    if(Math.abs(deltax)<0.3*documentWidth && Math.abs(deltay)<0.3*documentWidth)//设备宽度的0.3倍；
    return;
    //x
        ...
        for( var j = 1 ; j < 4 ; j ++ ){
            if( board[i][j] != 0 ){
                ...
                for( var k = 0 ; k < j ; k ++ ){
                    if( board[i][k] == 0 && noBlockHorizontal( i , k , j , board ) ){
                    ...
                    else if( board[i][k] == board[i][j] && noBlockHorizontal( i , k , j , board ) && !hasConflicated[i][k]){//可以移动的两个位置的值相等，并且之间没有障碍物；此时在将要移动到的位置没有发生过碰撞；
                        //move
                        showMoveAnimation( i , j , i , k );
                        //add
                        board[i][k] += board[i][j];
                        ...
                        score+=board[i][k];
                        updateScore(score);
                        hasConflicated[i][k]=true;//下一次在这个位置就不能发生碰撞了
                        continue;
                    }
                }
            }
